...
var tween_pause_mode := Tween.TWEEN_PAUSE_PROCESS
var timer_process_mode := Timer.PROCESS_MODE_ALWAYS
...
func create_timer(p_parent: Node, p_secs: float) -> Timer:
	...
	assert(timer_process_mode == Timer.PROCESS_MODE_ALWAYS or !p_parent.get_tree().paused)
	
	var timer = Timer.new()
	timer.process_mode = timer_process_mode
...
func create_tween(p_parent: Node) -> Tween:
	assert(!aborted)
	assert(p_parent != null and is_instance_valid(p_parent))
	assert(tween_pause_mode == Tween.TWEEN_PAUSE_PROCESS or !p_parent.get_tree().paused)
	
	var tween := p_parent.create_tween().set_pause_mode(tween_pause_mode)
